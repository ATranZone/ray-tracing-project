...
vec3 Camera::World_Position(const ivec2& pixel_index)
{
   //TODO
    vec3 result; 
    vec2 c_center = Cell_Center(pixel_index); //obtaining C_x and C_y from vec2 from cell_center(pixel_index)
    result = film_position + (c_center[0] * horizontal_vector) + (c_center[1] * vertical_vector); //implementing the given formula p + C_xu + C_yv
    return result;
}
